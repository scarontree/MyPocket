...
const catStats = computed(() => {
  const map = {}
  store.monthTx.forEach(t => { map[t.category] = (map[t.category] || 0) + t.amount })
    ...
    <div class="cat-bars" v-if="catStats.length">
      <div v-for="c in catStats" :key="c.id" class="cat-row">
        <div class="cat-icon" :style="{ background: c.bg, color: c.color }">
          <component :is="CAT_ICONS[c.id] || CAT_ICONS.other" :size="14" />
        </div>
        <div class="cat-info">
...
.stat-row { display: grid; grid-template-columns: repeat(3,1fr); gap: .75rem; margin-bottom: 2rem; }
.stat {
  background: var(--surface); border: 1px solid var(--border);
